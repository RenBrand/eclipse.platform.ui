...
public class TestModelLazyTreeContentProvider extends TestModelContentProvider
	...
	public TestModelLazyTreeContentProvider(
			TreeViewer<TestElement, TestElement> treeViewer) {
	...
	public void updateElement(TestElement parent, int index) {
		...
		if (parentElement.getChildCount() > index) {
			...
			treeViewer
					.setChildCount(childElement, childElement.getChildCount());
		}
	}
	...
	public void inputChanged(Viewer<? extends TestElement> viewer,
			TestElement oldInput, final TestElement newInput) {
